...
class CrawlerService
{
    ...
    protected function getRateLimitConfig(): array
    {
        ...
        try {
            $config = config('scannr.rate_limit', $defaults);
    ...
    protected function processInternalUrlAndGetResult(string $url, int $depth, string $source, string $element, array $scanElements, array $discoveryFlags = [], ?string $rel = null): ?array
    {
        ...
        $statusInt = is_numeric($result['status']) ? (int) $result['status'] : 0;
        if ($statusInt !== 429) {
    ...
    protected function configureJsRendering(ScanConfig $config, ?Closure $onMessage): void
    {
        ...
        if ($depCheck['available']) {
            ...
            $jsConfig = config('scannr.js_rendering', []);
            if (! empty($jsConfig)) {
